function [SSE_y, SSE_theta, mse_y, mse_theta, theta_hat] = PA(u, num_samples, sysd,noise_variance, theta_hat_zero, P_zero, gamma, alpha, main_title)
    rng(50);
    ...
    for i=1:num_samples
        ...
        switch i
            case 1 % at time t=1
                theta_hat(:, i) = theta_hat_zero + (gamma * phi_t)/(alpha + phi_t.' * phi_t) * (y_real(i) - phi_t.' * theta_hat_zero);
            otherwise
                theta_hat(:, i) = theta_hat(:, i-1) + (gamma * phi_t)/(alpha + phi_t.' * phi_t) * (y_real(i) - phi_t.' * theta_hat(:, i-1));
        end
    ...
    title("Output")
    saveas(gcf,'images/q2-8/' + main_title+ "_summary" + '.jpeg')
%     close all
    ...
    saveas(gcf,'images/q2-8/' + main_title + '.jpeg')
%     close all
...
%     saveas(gcf,'images/q2-8/' + main_title+ "_bode" + '.jpeg')
%     close all
