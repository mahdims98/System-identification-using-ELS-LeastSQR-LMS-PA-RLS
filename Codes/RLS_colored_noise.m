function [SSE_y, SSE_theta, mse_y, mse_theta, theta_hat] = RLS_colored_noise(u, num_samples, sysd,noise_variance, theta_hat_zero, P_zero, main_title)
    rng(50)
    ...
    for i=1:num_samples
        ...
        case 2
            ...
            noise_t = noise(i)+ 0.5 * noise(i-1);
        case 3
            phi_t = [-y_real(i-1:-1:i-2).',zeros(1,2), u(i-1:-1:i-2),zeros(1,2)].';
            noise_t = noise(i)+ 0.5 * noise(i-1)+0.25*noise(i-2);
        case 4
        ...
        phi(i, :) = phi_t.';
        y_real(i) = (phi_t.' * [den(2:end), num(2:end)].') + noise_t;
